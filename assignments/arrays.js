...
let oldCars = [];
for (let o = 0; o < inventory.length; o++) {
    if (inventory[o].car_year < 2000) {
        let old = inventory[o];
        oldCars.push(old);
...
for (let f = 0; f < inventory.length; f++) {
    if (inventory[f].car_make ===  'BMW' || inventory[f].car_make ===  'Audi'){
        BMWAndAudi.push(inventory[f].car_make);
